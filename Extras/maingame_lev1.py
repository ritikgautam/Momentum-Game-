...
def EndGame(mini,sec,milisec):
    ...
    end_screen.protocol("WM_DELETE_WINDOW", on_closing)
...
#load music
#running music
run_sound = pygame.mixer.Sound('Extras/img/music/era running.mp3')
# jump music
jump_sound = pygame.mixer.Sound('Extras/img/music/idle jump.mp3')
#background
music = pygame.mixer.music.load('Extras/img/music/background map/erangle background.mp3')
        ...
        for i in range(4):
            if i == 1:  # 2nd layer
                ...
                for snow in snow_list:
                    if x == 0:
                        snow.update(0.02)
                    pygame.draw.circle(window, DARK_GRAY, (snow.x + (x * SCREEN_WIDTH) -
                                       bg_scroll * bg_layer_speed[i], snow.y), snow.size)
            # if x==0 and i==3:               #4th layer
            #     player.draw()
            if bg_data[i][x] >= 0:
                window.blit(bg_list[i][bg_data[i][x]], ((
                    x * width) - bg_scroll * bg_layer_speed[i], 0))
